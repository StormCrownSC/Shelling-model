        ...
        try:
            ...
            for i in range(0, self.size_map, 1):
                if abs(i - y) <= self.optimal_distance:  # I don't now
                    for j in range(0, self.size_map, 1):
                        if self.distance(x, y, j, i) <= self.optimal_distance and self.Map[i][j] == type and not (x == j and y == i):
                            temp += 1
                        if self.distance(x, y, j, i) <= self.optimal_distance and self.another_agent(self.Map[i][j]) == type and not (x == j and y == i):
                            temp -= 13
