...
class Window(tk.Tk):
    def __init__(self, data):
        ...
        self.shelling_model = Shelling_model(self, data)
...
class Shelling_model(InputFrame):
    def __init__(self, window, data):
        super().__init__(window, data)
        self.generate_btn.bind('<Button-1>', self.enter_data)
    ...
    def read_data(self):
        try:
            self.data.size_population = int(self.size_population_elem.get())
            self.data.size_map = int(self.size_map_elem.get())
            self.data.optimal_distance = int(self.optimal_distance_elem.get())
            self.data.percentage_ratio = int(self.percentage_ratio_elem.get())
            self.data.value_tolerance = int(self.value_tolerance_elem.get())
            self.data.steps = int(self.steps_elem.get())
